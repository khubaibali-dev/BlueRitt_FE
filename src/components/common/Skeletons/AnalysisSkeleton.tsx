  ...
  return (
    <div className="relative z-10 p-6 sm:p-10 flex flex-col animate-pulse max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8 sm:mb-10">
        ...
        <div className="h-11 bg-white/10 rounded-full w-32"></div>
      </div>

      {/* Cards Skeleton */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 sm:mb-10">
        <div className="h-[500px] bg-white/5 rounded-[24px] border border-white/5"></div>
        <div className="h-[500px] bg-white/5 rounded-[24px] border border-white/5"></div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 sm:mb-10">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-40 bg-white/5 rounded-3xl border border-white/5"></div>
        ))}
      </div>

      {/* Table Skeleton */}
      <div className="bg-white/5 rounded-3xl border border-white/5 overflow-hidden">
        <div className="p-6 border-b border-white/5 h-16 bg-white/5"></div>
        <div className="p-6 space-y-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="flex gap-4 items-center">
              <div className="h-10 bg-white/5 rounded-lg flex-1"></div>
              <div className="h-10 bg-white/5 rounded-lg flex-1"></div>
              <div className="h-10 bg-white/5 rounded-lg flex-1"></div>
              <div className="h-10 bg-white/5 rounded-lg flex-1"></div>
            </div>
