...
const PremiumSearchBar: React.FC<PremiumSearchBarProps> = ({ 
  ...
  return (
    <div className={`w-full flex flex-col sm:flex-row items-center gap-3 sm:gap-0 sm:premium-search-bar sm:h-[72px] sm:p-2 sm:pr-2 ${className}`}>
      <div className="flex flex-1 items-center w-full premium-search-inner-mobile h-[56px] sm:h-full px-1 sm:px-0">
        ...
        <input
          ...
          className="inner-input flex-1 w-full bg-transparent border-none outline-none text-white text-[14px] sm:text-[16px] px-4 sm:px-4 placeholder-white/60"
        />
      </div>
      <PrimaryButton 
        className="search-ai-btn !w-full sm:!w-auto flex items-center justify-center gap-2 h-[48px] sm:h-[48px] !py-0 !px-6 sm:!px-8"
