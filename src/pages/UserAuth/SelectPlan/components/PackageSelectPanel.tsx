...
const PackageSelectPanel: React.FC<PackageSelectPanelProps> = ({
  ...
  return (
    <div className="w-full flex justify-end">
      ...
      <div className="w-full max-w-[450px] figma-card-border brand-card-bg rounded-[14px] p-6 lg:p-8 relative flex flex-col">
        ...
        <div className="w-full mb-6 relative z-10">
          <label className="block text-[#E2E8F0] text-[13px] font-medium mb-2">Package Type</label>
          <div className="flex bg-[#0A1626] rounded-[12px] p-1 border border-brand-border">
            <button
        ...
        {/* ── BILLING ── */}
        <div className="w-full mb-6 relative z-10">
          <label className="block text-[#E2E8F0] text-[13px] font-medium mb-2">Billing</label>
          <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-[#7A9ABF]">
              <Calendar size={18} />
            </div>
            <select
              value={billingCycle}
              onChange={(e) => onSelectBillingCycle(e.target.value as "Monthly" | "Annually")}
              className="w-full bg-[#0A1626] text-white text-[14px] rounded-[12px] border border-brand-border pl-12 pr-10 py-3.5 appearance-none focus:outline-none focus:border-brand-accent transition-colors cursor-pointer"
            >
              <option value="Monthly">Monthly</option>
              <option value="Annually">Annually</option>
            </select>
            <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-[#7A9ABF]">
              <ChevronDown size={18} />
            </div>
          </div>
        </div>
        ...
        <div className="w-full relative z-10">
          <label className="block text-[#E2E8F0] text-[13px] font-medium mb-3">Select Package</label>
          ...
          <div className="flex flex-col gap-3">
            {packages.map((pkg) => {
              ...
              return (
                ...
                >
                  {/* Icon Circle */}
                  <div className="pkg-icon-circle flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-white">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-[15px] font-semibold text-white tracking-wide">{pkg.name}</span>
                      {pkg.isPopular && (
                        <span className="pkg-popular-badge text-[10px] font-bold text-white px-2 py-[2px] rounded-full">
                          Popular
                        </span>
                      )}
                    </div>
                    <p className="text-[12px] text-[#7A9ABF] truncate">{pkg.tagline}</p>
                  </div>
                </button>
              );
            })}
