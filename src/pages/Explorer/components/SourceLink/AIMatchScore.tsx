...
const AIMatchScore: React.FC<AIMatchScoreProps> = ({ score }) => {
  const getGradientClass = (s: number) => {
    if (s >= 80) return 'gradient-border';
    if (s >= 51) return 'gradient-border-yellow';
    return 'gradient-border-red';
  };

  return (
    <div className="flex flex-col items-center justify-center shrink-0 py-4 lg:py-0 border-t border-white/5 lg:border-0 mr-12">
      <div className={`relative w-24 h-24 rounded-full flex flex-col items-center justify-center text-center ${getGradientClass(score)}`}>
