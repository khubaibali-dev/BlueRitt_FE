...
export interface AmazonProductInsightsResponse {
  metrics: {
    competing_products: string;
    avg_price: string;
    avg_num_reviews: string;
    avg_revenue: string;
...
const getAmazonSearchInsights = async ({
  ...
  isAmazonChoice?: boolean;
  categoryId?: string;
}): Promise<{ data: AmazonProductInsightsResponse }> => {
...
const aliBabaProductMatcher = async ({
  ...
  return api.get("/products/suppliers/discover/", {
    params: {
      asin: asin,
      query: title,
      country: country,
